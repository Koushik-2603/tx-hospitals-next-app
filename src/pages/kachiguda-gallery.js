...
export default function KachigudaGallery({ images }) {
    const [selectedImage, setSelectedImage] = useState(null);

    return (
        <>
            <Head>
                <title>Kachiguda Unit Inauguration Gallery | TX Hospitals</title>
                <meta name="description" content="View the full gallery of the Kachiguda Unit Inauguration at TX Hospitals." />
            </Head>
            <SecondaryLayout>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-10 font-inter">
                    ...
                    <div className="mb-10 text-left">
                        ...
                        <p className="mt-4 text-gray-500 max-w-2xl text-lg">
                            Explore the highlights and memorable moments from the grand inauguration of our Kachiguda facility.
                        </p>
...
export async function getStaticProps() {
    const imagesDir = path.join(process.cwd(), 'public', 'assets', 'NewsAndMedia', 'Kachiguda');
    let images = [];
    try {
        const filenames = fs.readdirSync(imagesDir);
        images = filenames.map(name => `/assets/NewsAndMedia/Kachiguda/${name}`);
    } catch (e) {
        console.error(e);
    }
