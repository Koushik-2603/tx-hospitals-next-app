...
export default function BanjaraHillsGallery({ images }) {
    const [selectedImage, setSelectedImage] = useState(null);

    return (
        <>
            <Head>
                <title>Banjara Hills Unit Inauguration Gallery | TX Hospitals</title>
                <meta name="description" content="View the full gallery of the Banjara Hills Unit Inauguration at TX Hospitals." />
            </Head>
            <SecondaryLayout>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-10 font-inter">
                    ...
                    <div className="mb-10 text-left">
                        ...
                        <p className="mt-4 text-gray-500 max-w-2xl text-lg">
                            Explore the highlights and memorable moments from the grand inauguration of our Banjara Hills facility.
                        </p>
                        ...
                        {images.map((src, index) => (
                            ...
                            >
                                <img 
                                    loading="lazy" 
                                    src={src}
                                    alt={`Banjara Hills ${index + 1}`}
                                    className="w-full h-56 md:h-64 object-cover transition-all duration-700 group-hover:scale-110 group-hover:opacity-90"
                                />
...
export async function getStaticProps() {
    const imagesDir = path.join(process.cwd(), 'public', 'assets', 'NewsAndMedia', 'Banjara Hills');
    let images = [];
    try {
        const filenames = fs.readdirSync(imagesDir);
        images = filenames.map(name => `/assets/NewsAndMedia/Banjara Hills/${name}`);
    } catch (e) {
        console.error(e);
    }
