export const locations = [
    {
        id: 1,
        name: "TX Hospitals, Uppal, Hyderabad",
        address: "# 2-6-71, Bharath Nagar Colony, Near Masjid Uppal, Hyderabad, Telangana, 500039 India",
        phone: "+91-40-43 108 108",
        mapCoords: [17.402553358453122, 78.56694358887127],
        image: "/assets/ContactUs/Uppal.jpg",
        slug: "uppal",
        seoTitle: "Contact TX Hospitals Uppal, Hyderabad | 24/7 NABH Multi-Specialty Hospital",
        seoDescription: "Get in touch with TX Hospitals Uppal, Hyderabad. NABH accredited, 24/7 multi-specialty hospital near Uppal Bus Stand. Call +91-40-43108108 for appointments.",
        seoKeywords: "TX Hospitals Uppal Hyderabad, Best Hospitals in uppal, top Hospitals in Uppal, Contact TX Hospitals Uppal, TX Hospitals Uppal phone number, TX Hospitals Uppal address, Best hospital in Uppal Hyderabad, 24/7 hospitals in Uppal, Multi-specialty hospital Uppal, NABH hospital in Uppal Hyderabad, Emergency hospital Uppal Hyderabad"
    },
    {
        id: 2,
        name: "TX Hospitals, Kachiguda, Hyderabad",
        address: "# 3-2-841/1, Kachiguda Station Rd, Mahalaxmi Nilayam, Kachiguda, Hyderabad, Telangana 500027",
        phone: "+91-40-48 108 108",
        mapCoords: [17.389777991297837, 78.49544286499808],
        image: "/assets/ContactUs/Kachiguda.jpg",
        slug: "kachiguda",
        seoTitle: "TX Hospitals Kachiguda, Hyderabad | 24/7 Multi-Specialty Hospital",
        seoDescription: "Reach TX Hospitals Kachiguda, Hyderabad for 24/7 multi-specialty medical care. Best accredited hospital with expert doctors & emergency services. Call now!",
        seoKeywords: "Contact TX Hospitals Kachiguda, Best hospital in Kachiguda Hyderabad, Multi-specialty hospital in Kachiguda, NABH hospitals in Kachiguda, 24/7 hospital in Kachiguda Hyderabad, Emergency hospital Kachiguda, Top doctors in Kachiguda, TX Hospitals phone number Kachiguda"
    },
    {
        id: 3,
        name: "TX Hospitals, Banjara Hills, Hyderabad",
        address: "# 8-2-680, Raichandani Construction, no:12, B Road, Sri Ram Nagar Colony, Banjara Hills, Hyderabad, Telangana 500028",
        phone: "040-66529999",
        mapCoords: [17.407846851900647, 78.44466631002469],
        image: "/assets/ContactUs/BanjaraUnit1.jpg",
        slug: "banjara-hills1",
        seoTitle: "TX Hospitals Banjara Hills, Hyderabad | 24/7 Multi-Specialty Hospital",
        seoDescription: "Get in touch with TX Hospitals Banjara Hills, Hyderabad. NABH accredited 24/7 multi-specialty hospital offering expert doctors, advanced care & emergency services.",
        seoKeywords: "TX Hospitals Banjara Hills Hyderabad, Contact TX Hospitals Banjara Hills, Best hospital in Banjara Hills Hyderabad, Multi-specialty hospital in Banjara Hills, NABH hospitals in Banjara Hills Hyderabad, 24/7 hospital in Banjara Hills, Emergency hospital in Banjara Hills, Top doctors in Banjara Hills, TX Hospitals contact number Banjara Hills"
    },
    {
        id: 4,
        name: "TX Children's Hospital, Banjara Hills, Hyderabad",
        address: "# 8-2-680, Raichandani Construction, no:12, B Road, Sri Ram Nagar Colony, Banjara Hills, Hyderabad, Telangana 500028",
        phone: "040-66529999",
        mapCoords: [17.407950683449737, 78.4449538935423],
        image: "/assets/ContactUs/BanjaraUnit2.jpg",
        slug: "banjara-hills2",
        seoTitle: "TX Children’s Hospital Banjara Hills, Hyderabad | Best Pediatric & Neonatal Care",
        seoDescription: "TX Children’s Hospital in Banjara Hills, Hyderabad offers expert pediatricians, NICU, PICU, and advanced child care services. 24/7 emergency support for children’s health.",
        seoKeywords: "TX Children’s Hospital Banjara Hills, Best children’s hospital in Banjara Hills, Pediatric hospital in Banjara Hills, Neonatal care hospital Banjara Hills, Best pediatrician in Banjara Hills Hyderabad, TX Hospitals children’s wing Banjara Hills, 24/7 children’s hospital in Banjara Hills, PICU and NICU hospital Banjara Hills, Child specialist doctors in Banjara Hills"
    },
    {
        id: 5,
        name: "TX Hospitals, Miyapur, Hyderabad",
        address: " Pillar No A 600, Plot 65 & 66, Miyapur Cross Road, Mathrusree Nagar, Hafeezpet, Hyderabad, Telangana 500059",
        phone: "",
        mapCoords: [17.49594147086547, 78.36466448274872],
        image: "",
        slug: "miyapur",
        seoTitle: "TX Hospitals Miyapur, Hyderabad | 24/7 Multi-Specialty Hospital",
        seoDescription: "Visit TX Hospitals Miyapur, Hyderabad for 24/7 multi-specialty medical care. Best accredited hospital with expert doctors & emergency services. Call now!",
        seoKeywords: "Contact TX Hospitals Miyapur, Best hospital in Miyapur Hyderabad, Multi-specialty hospital in Miyapur, NABH hospitals in Miyapur, 24/7 hospital in Miyapur Hyderabad, Emergency hospital Miyapur, Top doctors in Miyapur, TX Hospitals phone number Miyapur"
    }
];