...
const BanjaraHillsNewHero = () => {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                ...
                console.error("Error fetching options in BanjaraHillsNewHero:", error);
            }
        };
        fetchAllData();
    }, []);
    ...
    const doctorOptions = getDoctorsForLocation('Banjara').map(doctor =>
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                subject: `New Inquiry from Banjara Hills Hero Section (${activeTab === 'appointment' ? 'Book Appointment' : activeTab === 'second-opinion' ? 'Second Opinion' : 'Book Health Checkup'})`,
                html: `
                    <h3>New Hero Section Inquiry - Banjara Hills Landing Page</h3>
            ...
            setFormData({ name: '', phone: '', location: 'Banjara Hills', doctor: '', date: '' });
    ...
    return (
        <section id="home" className="relative flex overflow-hidden w-full -mt-2 md:mt-4" style={{ minHeight: '660px' }}>
            <div className="relative flex-grow hidden lg:block" style={{ background: 'linear-gradient(to bottom, #5A9BD3 20%, #5E5C5D 80%)' }}>
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, #5A9BD3 20%, #5E5C5D 80%)' }}>
                    <Image
                        src="/assets/banjara-hills/Banjara-Hills.png"
                        alt="TX Hospitals Banjara Hills and Children Hospital Buildings"
                        layout="fill"
                        className="object-cover object-center 2xl:object-contain 2xl:object-right"
                        priority
                    />
                </div>
            </div>
...
export default BanjaraHillsNewHero;
